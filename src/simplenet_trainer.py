...
if __name__ == "__main__":
    ...
    import simplenet
    import patch_sampling
    from simplenet import LR_SCHEDULE
    from dataset import label_name

class SimpleNetTrainer(trainer.Trainer):
    def __init__(self):
        metric_names = ['Loss','L2','Accuracy']
        super(SimpleNetTrainer, self).__init__(metric_names)

        input_var = T.tensor4('inputs')
        target_var = T.ivector('targets')

        logging.info("Defining network")
        net = simplenet.SimpleNet(input_var)
        self.network = net
        train_fn, val_fn, l_r = simplenet.define_updates(self.network, input_var, target_var)
