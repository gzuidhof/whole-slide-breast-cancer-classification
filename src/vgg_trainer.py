...
class VGGTrainer(trainer.Trainer):
    def __init__(self):
        metric_names = ['Loss','L2','Accuracy', 'BinaryAccuracy']
        super(VGGTrainer, self).__init__(metric_names)

        input_var = T.tensor4('inputs')
        target_var = T.ivector('targets')

        logging.info("Defining network")
        net = vggnet.build_model(input_var)
...
if __name__ == "__main__":
    ...
    def genny(x):
        n = x[0]
        return np.zeros((n, 3, 224, 244), dtype=np.float32), np.zeros(n, dtype=np.int32), ['-']*n
    

    train_generator, validation_generator = patch_sampling.prepare_custom_sampler(mini_subset=False)#, override_cache_size=1)
    #train_generator = validation_generator = genny
