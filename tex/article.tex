...
\begin{abstract}
This paper presents and evaluates a method for characterization of breast malignant lesions in histopathological breast tissue images. The goal of this method is to best classify digitized whole-slide hematoxylin and eosin (H\&E) stained whole-slide images (WSIs) into three classes: normal/benign, ductal carcinoma in situ (DCIS) and invasive ductal carcinoma (IDC). A convolutional neural network (CNN) was trained on small patches of these images, labeled with the class of the center pixel. To distinguish between the DCIS and IDC class, the information available in these small patches is often not sufficient. A second CNN was, therefore, trained on the output of the aforementioned network with a larger patch size to capture a larger context. Sliding this classifier across a whole-slide image (WSI) yields a probability map. From this probability map structural and morphological features were extracted, which were used to train a final classifier to predict the label for the whole-slide image. The system is evaluated on a dataset containing 221 WSIs of breast tissue using ROC analysis. The results show that the distinction between benign and cancerous WSIs can be predicted with an accuracy of just over 90\%, whereas the three class problem yields an accuracy of 76.6\%, demonstrating the potential for application in routine diagnostics.
\end{abstract}
...
%\textit{TODO: introduce DCIS, IDC a little bit at least.} 
...
\input{tables/table_accuracy_224}
...
\input{tables/table_accuracy_stacked}
...
\vspace{-0.435cm}
\centering{}
\hspace{-0.1cm}\includegraphics[width=3.5in]{figures/misclassifications.png}
